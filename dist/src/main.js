...
io.on('connection', (socket) => {
    ...
    const name = socket.handshake.query.name; // TODO: define this query as a type
    ...
    socket.on('global:changeName', (name) => {
    ...
    socket.on("room:create", async (slug) => {
    ...
    socket.on("room:chatMessage", async (message) => {
        if (!client.roomSlug) {
    ...
    socket.on("room:startGame", async (data) => {
    ...
    socket.on("room:provideAnswers", async (data) => {
        assert(data, "Data is required");
        assert(data.slug, "Slug is required");
    ...
    socket.on("room:voteGoToLobby", async (data) => {
